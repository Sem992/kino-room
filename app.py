...
def clean_url(url_str):
    if not url_str:
        return "https://via.placeholder.com/300x450?text=Нет+постера"
    cleaned = str(url_str).strip().replace('\n', '').replace('\r', '').replace('\t', '')
...
if st.session_state.user_role is not None:
    ...
    if st.session_state.current_page == "catalog":
        ...
        if st.session_state.random_movie:
            if st.session_state.random_movie == "empty":
                ...
                rm_poster = clean_url(rm.get('poster_url'))
                st.markdown(f"""
                    <div style="background-color: #FFF; border: 2px solid #E50914; padding: 15px; border-radius: 8px; margin-top: 10px; display: flex; gap: 15px; align-items: center;">
                        <img src="{rm_poster}" style="width: 80px; max-height: 120px; object-fit: cover; border-radius: 4px;">
                        <div>
            ...
            for chunk in chunks:
                ...
                for index, movie in enumerate(chunk):
                    with cols[index]:
                        ...
                        status_badge = ""
                        if m_status == "watched":
                            status_badge = "<div style='margin-top:6px;'><span style='background-color:#28A745; color:white; padding:2px 6px; border-radius:4px; font-size:11px;'>✅ Просмотрено</span></div>"
                        elif m_status == "watchlist":
                            status_badge = "<div style='margin-top:6px;'><span style='background-color:#FFC107; color:black; padding:2px 6px; border-radius:4px; font-size:11px;'>📌 В планах</span></div>"
                        ...
                        p_url = clean_url(movie.get('poster_url'))

                        card_html = (
                            f'<div class="movie-card" style="position:relative; background-color:#ffffff; padding:15px; border-radius:10px; border:1px solid #E0E0E0; margin-bottom:15px; text-align:center;">'
                            f'{rec_badge}'
                            f'<img src="{p_url}" style="width:100%; max-height:380px; object-fit:cover; border-radius:8px; margin-bottom:10px;">'
                            f'<h3 style="color:#2B2B2B !important; margin: 5px 0; font-size:20px; text-align:center;">{movie["title"]}</h3>'
                            f'<div><span style="background-color:#E50914; color:white; padding:3px 10px; border-radius:4px; font-size:12px; font-weight:bold;">{movie["category"]}</span></div>'
                            f'{status_badge}'
                            f'</div>'
                        )
                    ...
                    if st.form_submit_button("Сохранить и добавить в каталог"):
                        if new_title and new_description:
                            save_local_movie({
                                ...
                                "poster_url": clean_url(new_poster),
            ...
            for r_chunk in r_chunks:
                ...
                for r_idx, r_movie in enumerate(r_chunk):
                    with r_cols[r_idx]:
                        ...
                        r_poster = clean_url(r_movie.get('poster_url'))

                        card_html = (
                            f'<div class="movie-card">'
                            f'<img src="{r_poster}" style="width:100%; max-height:380px; object-fit:cover; border-radius:8px; margin-bottom:10px;">'
                            f'<h3 style="color:#2B2B2B !important; margin: 5px 0; font-size:20px; text-align:center;">{r_movie["title"]}</h3>'
                            f'<div><span style="background-color:#E50914; color:white; padding:3px 10px; border-radius:4px; font-size:12px; font-weight:bold;">{r_movie["category"]}</span></div>'
                            f'{status_badge}'
                            f'</div>'
                        )
                        st.markdown(card_html, unsafe_allow_html=True)
            ...
            if not watched_movies:
                ...
                for w_chunk in w_chunks:
                    ...
                    for w_idx, w_movie in enumerate(w_chunk):
                        with w_cols[w_idx]:
                            w_poster = clean_url(w_movie.get('poster_url'))
                            card_html = (
                                f'<div class="movie-card">'
                                f'<img src="{w_poster}" style="width:100%; max-height:380px; object-fit:cover; border-radius:8px; margin-bottom:10px;">'
                                f'<h3 style="color:#2B2B2B !important; margin: 5px 0; font-size:20px; text-align:center;">{w_movie["title"]}</h3>'
                                f'<div><span style="background-color:#28A745; color:white; padding:3px 10px; border-radius:4px; font-size:12px; font-weight:bold;">✅ Просмотрено</span></div>'
                                f'</div>'
                            )
                            st.markdown(card_html, unsafe_allow_html=True)
            ...
            if not wish_movies:
                ...
                for wl_chunk in wl_chunks:
                    ...
                    for wl_idx, wl_movie in enumerate(wl_chunk):
                        with wl_cols[wl_idx]:
                            wl_poster = clean_url(wl_movie.get('poster_url'))
                            card_html = (
                                f'<div class="movie-card">'
                                f'<img src="{wl_poster}" style="width:100%; max-height:380px; object-fit:cover; border-radius:8px; margin-bottom:10px;">'
                                f'<h3 style="color:#2B2B2B !important; margin: 5px 0; font-size:20px; text-align:center;">{wl_movie["title"]}</h3>'
                                f'<div><span style="background-color:#FFC107; color:black; padding:3px 10px; border-radius:4px; font-size:12px; font-weight:bold;">📌 В планах</span></div>'
                                f'</div>'
                            )
                            st.markdown(card_html, unsafe_allow_html=True)
        ...
        with tab_achievements:
            ...
            achievements_config = [
                ...
                {"target": 7, "cur": cnt_watch_film, "name": "Вошла во вкус", "desc": "Посмотреть 7 фильмов",
                 "emoji": "😋"},
                {"target": 10, "cur": cnt_watch_film, "name": "Смотрю лучше, чем сплю", "desc": "Посмотреть 10 фильмов",
                 "emoji": "🍿"},
                ...
                {"target": 5, "cur": cnt_watch_total, "name": "Киномарафонец", "desc": "Посмотреть 5 тайтлов всего",
                 "emoji": "🧭"},
                {"target": 7, "cur": cnt_watch_total, "name": "Кинолюбитель", "desc": "Посмотреть 7 тайтлов всего",
                 "emoji": "❤️"},
                {"target": 10, "cur": cnt_watch_total, "name": "Кинопутешественник",
                 "desc": "Посмотреть 10 тайтлов всего", "emoji": "🌍"},
                {"target": 15, "cur": cnt_watch_total, "name": "Почетный гость Кинозала",
                 "desc": "Посмотреть 15 тайтлов всего", "emoji": "🥂"},
                {"target": 20, "cur": cnt_watch_total, "name": "Хранитель пульта",
                 "desc": "Посмотреть 20 тайтлов всего", "emoji": "🎮"},
                {"target": 25, "cur": cnt_watch_total, "name": "Друг режиссёра", "desc": "Посмотреть 25 тайтлов всего",
                 "emoji": "🎬"},
                {"target": 30, "cur": cnt_watch_total, "name": "Хранитель кадров",
                 "desc": "Посмотреть 30 тайтлов всего", "emoji": "🎞️"},
                {"target": 35, "cur": cnt_watch_total, "name": "Амбассадор Кинопоиска",
                 "desc": "Посмотреть 35 тайтлов всего", "emoji": "🦊"},
                {"target": 40, "cur": cnt_watch_total, "name": "Покоритель экранов",
                 "desc": "Посмотреть 40 тайтлов всего", "emoji": "🚀"},
                {"target": 45, "cur": cnt_watch_total, "name": "Легенда просмотра",
                 "desc": "Посмотреть 45 тайтлов всего", "emoji": "🌟"},
                {"target": 50, "cur": cnt_watch_total, "name": "Живёт в кинозале",
                 "desc": "Посмотреть 50 тайтлов всего", "emoji": "🏠"},
                {"target": 55, "cur": cnt_watch_total, "name": "Спилберг нервно курит",
                 "desc": "Посмотреть 55 тайтлов всего", "emoji": "🚬"},
                {"target": 60, "cur": cnt_watch_total, "name": "Властелин тайтлов",
                 "desc": "Посмотреть 60 тайтлов всего", "emoji": "🌋"},
                ...
                {"target": 30, "cur": cnt_rate_film, "name": "Властелин тайтлов", "desc": "Оценить 30 фильмов",
                 "emoji": "🔮"},
                ...
                {"target": 30, "cur": cnt_rate_mult, "name": "Легендарный судья анимации",
                 "desc": "Оценить 30 мультфильмов", "emoji": "🏛️"},

                # --- ОЦЕНИТЬ ТАЙТЛЫ (ВСЕГО) ---
                {"target": 5, "cur": cnt_rate_total, "name": "Младший оценщик", "desc": "Оценить 5 тайтлов всего",
                 "emoji": "🐣"},
                {"target": 7, "cur": cnt_rate_total, "name": "Есть что сказать", "desc": "Оценить 7 тайтлов всего",
                 "emoji": "🗣️"},
                {"target": 10, "cur": cnt_rate_total, "name": "Уверенный критик", "desc": "Оценить 10 тайтлов всего",
                 "emoji": "😎"},
                {"target": 15, "cur": cnt_rate_total, "name": "Формирователь вкуса", "desc": "Оценить 15 тайтлов всего",
                 "emoji": "🍷"},
                {"target": 20, "cur": cnt_rate_total, "name": "Куратор рейтингов", "desc": "Оценить 20 тайтлов всего",
                 "emoji": "🗂️"},
                {"target": 25, "cur": cnt_rate_total, "name": "Эксперт впечатлений", "desc": "Оценить 25 тайтлов всего",
                 "emoji": "🗺️"},
                {"target": 30, "cur": cnt_rate_total, "name": "Неподкупное жюри", "desc": "Оценить 30 тайтлов всего",
                 "emoji": "🔒"},
                {"target": 35, "cur": cnt_rate_total, "name": "Профи оценок", "desc": "Оценить 35 тайтлов всего",
                 "emoji": "💎"},
                {"target": 40, "cur": cnt_rate_total, "name": "Мастер вкуса", "desc": "Оценить 40 тайтлов всего",
                 "emoji": "👑"},
                {"target": 45, "cur": cnt_rate_total, "name": "Энциклопедия оценок", "desc": "Оценить 45 тайтлов всего",
                 "emoji": "📖"},
                {"target": 50, "cur": cnt_rate_total, "name": "Абсолютный авторитет",
                 "desc": "Оценить 50 тайтлов всего", "emoji": "🔱"},
                {"target": 55, "cur": cnt_rate_total, "name": "Министерство культуры",
                 "desc": "Оценить 55 тайтлов всего", "emoji": "🏛️"},
                {"target": 60, "cur": cnt_rate_total, "name": "Верховный суд тайтлов",
                 "desc": "Оценить 60 тайтлов всего", "emoji": "⚖️"},
                ...
                {"target": 15, "cur": cnt_rev_film, "name": "Голос кинозала", "desc": "Написать рецензию на 15 фильмов",
                 "emoji": "📢"},
                ...
                {"target": 10, "cur": cnt_rev_mult, "name": "Летописец мультмиров",
                 "desc": "Написать рецензию на 10 мультфильмов", "emoji": "🗺️"},
                ...
                {"target": 20, "cur": cnt_rev_mult, "name": "Маг рецензий",
                 "desc": "Написать рецензию на 20 мультфильмов", "emoji": "🧙‍♂️"},
                {"target": 25, "cur": cnt_rev_mult, "name": "Архивариус детства",
                 "desc": "Написать рецензию на 25 мультфильмов", "emoji": "🗄️"},
                ...
                {"target": 5, "cur": cnt_rev_total, "name": "Начинающий автор",
                 "desc": "Написать рецензию на 5 тайтлов всего", "emoji": "✍️"},
                {"target": 7, "cur": cnt_rev_total, "name": "Любитель обзоров",
                 "desc": "Написать рецензию на 7 тайтлов всего", "emoji": "📂"},
                {"target": 10, "cur": cnt_rev_total, "name": "Аналитик с дивана",
                 "desc": "Написать рецензию на 10 тайтлов всего", "emoji": "🛋️"},
                {"target": 15, "cur": cnt_rev_total, "name": "Киноблогер",
                 "desc": "Написать рецензию на 15 тайтлов всего", "emoji": "📸"},
                {"target": 20, "cur": cnt_rev_total, "name": "Свободный микрофон",
                 "desc": "Написать рецензию на 20 тайтлов всего", "emoji": "🎙️"},
                {"target": 25, "cur": cnt_rev_total, "name": "Повелитель текста",
                 "desc": "Написать рецензию на 25 тайтлов всего", "emoji": "⌨️"},
                {"target": 30, "cur": cnt_rev_total, "name": "Голос сообщества",
                 "desc": "Написать рецензию на 30 тайтлов всего", "emoji": "📣"},
                {"target": 35, "cur": cnt_rev_total, "name": "Мыслитель",
                 "desc": "Написать рецензию на 35 тайтлов всего", "emoji": "💭"},
                {"target": 40, "cur": cnt_rev_total, "name": "Мастер пера",
                 "desc": "Написать рецензию на 40 тайтлов всего", "emoji": "🪶"},
                {"target": 45, "cur": cnt_rev_total, "name": "Главный редактор",
                 "desc": "Написать рецензию на 45 тайтлов всего", "emoji": "💼"},
                {"target": 50, "cur": cnt_rev_total, "name": "Хранитель рецензий",
                 "desc": "Написать рецензию на 50 тайтлов всего", "emoji": "📦"},
                {"target": 55, "cur": cnt_rev_total, "name": "Живая энциклопедия",
                 "desc": "Написать рецензию на 55 тайтлов всего", "emoji": "🧬"},
                {"target": 60, "cur": cnt_rev_total, "name": "Абсолютный обозреватель тайтлов",
                 "desc": "Написать рецензию на 60 тайтлов всего", "emoji": "🌌"},
            ]
    ...
    if st.session_state.current_page == "movie_view" and st.session_state.selected_movie_id is not None:
        ...
        if movie:
            ...
            with col_view1:
                st.image(clean_url(movie.get('poster_url')), use_container_width=True)
            with col_view2:
